...
%type <number> expr_arit soma_sub mult_div term
%type <boole> expr_log factor
%type program line assignment if jump_if else
...
assignment: ID RECEIVE expr_arit DONE { 
				if (!hasError) {
            ...
            // To catch all errors
		  	| ID RECEIVE expr_arit error { yyerrok; yyclearin; }
			| ID RECEIVE error DONE { yyerrok; yyclearin; }
			| ID error expr_arit DONE { yyerrok; yyclearin; }
			| error RECEIVE expr_arit DONE { yyerrok; yyclearin; }
			;

expr_arit: soma_sub { $$ = $1; }
		| mult_div { $$ = $1; }
        | LEFTPAR expr_arit RIGHTPAR { $$ = $2; }
		| term { $$ = $1; }
		;

soma_sub: expr_arit PLUS expr_arit { $$ = $1 + $3; }
    	| expr_arit MIN  expr_arit { $$ = $1 - $3; }
		;

mult_div: expr_arit MULT expr_arit { $$ = $1 * $3; }
...
if: IF LEFTPAR expr_log RIGHTPAR jump_if LEFTKEYS program RIGHTKEYS {
        ifElse = $3; 
        if(!ifElse) {
...
expr_log: term LESS    term     { $$ = $1 < $3;  }
        | term GREAT   term     { $$ = $1 > $3;  }
        | term LEQUAL  term     { $$ = $1 <= $3; }
        | term GEQUAL  term     { $$ = $1 >= $3; }
        | term EQUAL   term     { $$ = $1 == $3; }
        | term NEQUAL  term     { $$ = $1 != $3; }
        | expr_log AND expr_log { $$ = $1 && $3; }
        | expr_log OR  expr_log { $$ = $1 || $3; }
        |          NOT expr_log {    $$ = !$2;   }
        | LEFTPAR expr_log RIGHTPAR {  $$ = $2;  }
        | factor                    {  $$ = $1;  }
		;
